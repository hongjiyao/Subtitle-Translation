...
def create_ui():
    ...
    with gr.Blocks(title="字幕翻译工具") as demo:
        ...
        def add_to_queue(video):
            if not video:
                return queue_manager.get_queue(), "请先上传视频文件", f"当前队列: {len(queue_manager.video_queue)} 个文件", None
            ...
            return queue_manager.get_queue(), f"已添加 {len(files)} 个文件到队列", f"当前队列: {count} 个文件", None
        
        def clear_queue():
            queue_manager.clear_queue()
            return queue_manager.get_queue(), "队列已清空", "当前队列: 0 个文件", None
        
        def process_queue():
            if not queue_manager.video_queue:
                return queue_manager.get_queue(), "队列为空", f"当前队列: {len(queue_manager.video_queue)} 个文件", None
            ...
            remaining = len(queue_manager.video_queue) - 1
            return queue_manager.get_queue(), "\n".join(logs), f"当前队列: {remaining} 个文件", output if success else None
